...
class ChatbotBase:
    ...
    def setup_agent(self, user_id: str, chatbot_name: str = "AI助手", 
        ...
        提醒方法支援兩種：
        - notification（通知）：一般通知提醒，適用於日常事務
        - alarm（鬧鐘）：鬧鐘提醒，適合需要更強烈提醒的場景，如重要會議、吃藥等
        
        若主人明確提到「鬧鐘」或「alarm」，使用 alarm 方法；
        若主人明確提到「通知」或「notification」，使用 notification 方法；
        若主人未明確指定，則根據事件的重要性和緊急程度判斷：
        - 重要或緊急事件（如會議、吃藥、截止日期）使用 alarm
        - 一般事務（如購物、一般提醒）使用 notification
        ...
        主人用什麼語言跟你交談，你就用什麼語言回答，除非主人明確要求使用其他語言。
        """
